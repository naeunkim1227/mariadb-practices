...
(select count(*) from employees where gender = 'm') as '남직원' from dual;
...
select count(*) as '직원 수' from salaries where to_date = '9999-01-01'; 


-- 문제5. 부서는 총 몇 개가 있나요?
select distinct count(*) as department_count from departments;

-- 문제6.현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
select distinct count(*) as dept_mng_count from dept_manager;

-- 문제7.전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요.
select distinct name from dept order by length(name) desc;
...
select count(*) as Engineer_count from titles where title like '%Engineer';
